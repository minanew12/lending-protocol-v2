...
@pytest.fixture
def x2y2_proxy(x2y2_proxy_contract_def, p2p_nfts_weth):
    aave_pool_address_provider = "0x2f39d218133AFaB8F2B819B1066c7E434Ad94E9e"
    proxy = x2y2_proxy_contract_def.deploy(p2p_nfts_weth.address, aave_pool_address_provider)
...
def test_initial_state(aave, x2y2_proxy, weth, p2p_nfts_weth, borrower):
    assert x2y2_proxy.POOL() == aave.address
    assert x2y2_proxy.ADDRESSES_PROVIDER() == aave.ADDRESSES_PROVIDER()


def test_pay_loan(x2y2_proxy, weth, borrower, owner, captainz):
    ...
    # weth.transfer(x2y2_proxy.address, amount, sender=borrower)
    # weth.approve(approved, amount, sender=borrower)
...
def test_refinance(
    ...
    weth.approve(x2y2_proxy.address, amount + flash_loan_fee, sender=borrower)
    ...
    x2y2_proxy.refinance_loan(x2y2_contract, approved, loan_id, amount, signed_offer, token_id, sender=borrower)
